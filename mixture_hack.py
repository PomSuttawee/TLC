...
def calculate_concentration(mixture: mt.Mixture, calibration: list):
    ...
    equations = []
    # Equation for Each Peak of Mixture
    for i in range(len(mixture.peak_area['R'])):
        if i >= len(calibration):
            break
        
        coef = [calib.peaks[i].best_fit_line['R'][0] for calib in calibration]
        const = sum([calib.peaks[i].best_fit_line['R'][1] for calib in calibration])
